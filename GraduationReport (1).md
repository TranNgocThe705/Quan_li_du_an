...
DocPlant là một ứng dụng di động được xây dựng nhằm hỗ trợ người dùng trong việc nhận diện cây trồng thông qua hình ảnh, lưu trữ hồ sơ cá nhân về cây trồng, lên lịch chăm sóc và kết nối cộng đồng để chia sẻ kinh nghiệm. Ứng dụng được phát triển trên nền tảng React Native, tích hợp dịch vụ Firebase (Authentication, Firestore, Storage) cho phần dữ liệu và backend Node.js cho các dịch vụ đặc thù như xử lý ảnh và API. Đồng thời, mô hình nhận diện được tích hợp thông qua TensorFlow Lite để phù hợp với việc tra cứu nhanh ngay trên thiết bị.

Trong quá trình thực hiện đồ án, em đã học hỏi nhiều điều về kỹ thuật, thiết kế giao diện và quy trình phát triển phần mềm; sự hướng dẫn tận tình, những góp ý quý báu và chỉ dẫn chuyên môn từ GV hướng dẫn PGS.TS Hoàng Hữu Việt đã giúp em từng bước hoàn thiện sản phẩm ban đầu của mình. 

Em xin chân thành cảm ơn !

# CHƯƠNG 1: MÔ TẢ BÀI TOÁN VÀ YÊU CẦU HỆ THỐNG DOCPLANT

## 1.1. Bài toán của DocPlant
Trong thực tế, người dùng, người nông dân và những người yêu cây gặp khó khăn trong việc nhận diện loài cây, phát hiện sớm bệnh lý và quản lý chăm sóc cây trồng. Nhiều người thiếu kiến thức chuyên môn nên khó tự chăm sóc, điều trị hoặc bổ sung dinh dưỡng đúng cách. DocPlant là ứng dụng di động hỗ trợ nhận diện cây qua hình ảnh, lưu trữ và quản lý hồ sơ cây, cung cấp hướng dẫn chăm sóc và nền tảng cộng đồng để chia sẻ kiến thức.

## 1.2. Khảo sát nghiệp vụ
Khảo sát nghiệp vụ giúp nhận diện các yêu cầu cốt lõi, tương tác người dùng và quy trình chính của ứng dụng.

[Cần hình: Biểu đồ ca sử dụng mô tả các actor chính và luồng nghiệp vụ]

[Cần hình: Các người dùng mẫu - Chủ sở hữu cây / Thành viên cộng đồng / Quản trị viên (3 thẻ người dùng mẫu)]
...
- Chức năng chính:
	- Nhận diện cây từ ảnh chụp.
	- Quản lý hồ sơ cây (tạo, sửa, xóa, lịch chăm sóc).
...
**1. Quy trình nhận diện cây và phát hiện bệnh:**
- Tải lên / chụp ảnh -> tiền xử lý (resize, normalize) -> dự đoán bằng TFLite (on-device) -> hiển thị kết quả với độ tin cậy -> lưu vào hồ sơ (scans collection).
- Nếu phát hiện bệnh: gợi ý tạo Treatment Schedule ngay lập tức.

[Cần hình: Sơ đồ luồng minh họa quy trình nhận diện ảnh]

**2. Quy trình quản lý Treatment Schedule (Lịch điều trị bệnh):**
- Sau khi scan phát hiện bệnh -> Load treatment templates từ Firestore (dựa trên diseaseId)
- Người dùng xác nhận tạo lịch -> Tạo `treatmentSchedules` document
- Hệ thống tự động tạo các `plantTasks` (nhiệm vụ điều trị) theo template
- Người dùng nhận thông báo nhắc nhở theo lịch -> Đánh dấu hoàn thành từng task
- Theo dõi tiến độ (completedTasks/totalTasks) và cập nhật trạng thái

**3. Quy trình quản lý Plant Care Tasks (Nhiệm vụ chăm sóc định kỳ):**
- Người dùng tạo hồ sơ cây (myPlants) -> Thêm care tasks (tưới nước, bón phân, cắt tỉa)
- Cấu hình tần suất: daily, weekly, interval, hoặc custom (daysOfWeek)
- Hệ thống tự động tính `nextDueDate` sau khi hoàn thành task
- Gửi notification nhắc nhở trước thời hạn
- Hỗ trợ recurring: task tự động lặp lại theo lịch đã cấu hình

**4. Quy trình quản lý hồ sơ cây:**
- Tạo hồ sơ (myPlants) với thông tin: tên, loài, hình ảnh, ghi chú
- Thêm lịch chăm sóc (plantCareTasks) và treatment schedules (nếu có)
- Cập nhật trạng thái định kỳ -> Nhận thông báo nhắc nhở
- Xem lịch sử scan và disease history timeline

**5. Quy trình tương tác cộng đồng:**
- **User Posts**: Người dùng tạo bài viết (posts) -> Upload ảnh -> Thêm nội dung/tags -> Publish
- **Expert Articles**: Admin/Expert tạo bài viết chính thức (articles) -> Status = pending -> Admin duyệt (approved/rejected) -> Hiển thị với badge "Official"
- Tương tác: Bình luận, like, share, bookmark
- Báo cáo vi phạm: User report -> Tạo `reports` document -> Admin xử lý

**6. Quy trình quản trị (Admin):**
- Duyệt bài viết chuyên gia (articles): Xem preview -> Approve/Reject -> Pin/Unpin
- Xử lý report: Xem danh sách -> Review content -> Hide/Delete/Ban user -> Ghi audit log
- Quản lý user: Khóa/Mở khóa tài khoản, phân quyền (user/expert/admin)
- Quản lý templates: CRUD cho disease templates, treatment templates, plant care templates
- Thống kê: Dashboard hiển thị metrics (users, posts, reports, active treatments)

## 1.3. Một số website / app tương tự DocPlant
Sau khi khảo sát thị trường, một số sản phẩm tương tự có thể tham khảo:

### 1.3.1. PlantNet
- Giới thiệu: Ứng dụng nhận diện cây dựa trên cộng đồng và công nghệ học máy. Người dùng có thể gửi ảnh để xác định loài và tham gia đóng góp dữ liệu.
- Điểm mạnh: Cộng đồng lớn, đa dạng dữ liệu.
[Cần hình: Ảnh màn hình ứng dụng PlantNet (trang chủ hoặc tính năng nhận diện)]

### 1.3.2. PictureThis
- Giới thiệu: Ứng dụng thương mại sử dụng mô hình học máy để nhận diện cây và cung cấp gợi ý chăm sóc.
- Điểm mạnh: Gợi ý chăm sóc cụ thể và giao diện thân thiện.
[Cần hình: Ảnh màn hình PictureThis (kết quả nhận diện hoặc gợi ý chăm sóc)]

### 1.3.3. iNaturalist
- Giới thiệu: Nền tảng khoa học công dân giúp nhận diện và ghi nhận dữ liệu về đa dạng sinh học; có các tính năng xã hội và có tập dữ liệu lớn.
- Điểm mạnh: Tích hợp với nghiên cứu khoa học, nhiều đóng góp từ cộng đồng.
[Cần hình: Ảnh màn hình iNaturalist (tải lên hoặc kết quả nhận diện)]
...
#### Bảng 1.4.1-a: Chức năng của Người dùng (Chủ sở hữu cây)
| STT | Chức năng | Mô tả |
|---|---|---|
| 1 | Đăng ký/Đăng nhập | Đăng ký tài khoản, đăng nhập bằng email hoặc Google/Facebook. |
| 2 | Nhận diện cây & Phát hiện bệnh | Tải lên/chụp ảnh để nhận diện loài và phát hiện bệnh (TFLite on-device), hiển thị kết quả với độ tin cậy và severity. |
| 3 | Quản lý hồ sơ cây | Tạo, xem, chỉnh sửa, xóa hồ sơ cây (myPlants); xem disease history timeline và scan history. |
| 4 | Treatment Schedule | Tạo lịch điều trị bệnh từ kết quả scan, theo dõi tiến độ (completedTasks/totalTasks), đánh dấu hoàn thành từng task. |
| 5 | Plant Care Tasks | Tạo và quản lý nhiệm vụ chăm sóc định kỳ (tưới nước, bón phân, cắt tỉa) với recurring support (daily/weekly/interval/custom). |
| 6 | Tương tác cộng đồng | Tạo bài viết (posts), bình luận, like, chia sẻ, bookmark và báo cáo bài viết vi phạm. |
| 7 | Thông báo & Nhắc nhở | Nhận thông báo nhắc nhở cho plant care tasks, treatment tasks, và scheduled notifications. |

[Cần hình: Ảnh minh họa giao diện hồ sơ cây hoặc lịch chăm sóc]

#### Bảng 1.4.1-b: Chức năng của Người chia sẻ (Thành viên cộng đồng)
| STT | Chức năng | Mô tả |
|---|---|---|
| 1 | Tạo bài viết cộng đồng (Posts) | Tạo bài viết nhiều phần (văn bản, ảnh, tag) để chia sẻ kinh nghiệm chăm sóc cây, phát hiện bệnh, tips & tricks. |
| 2 | Đọc bài viết chuyên gia (Articles) | Xem các bài viết chính thức từ Admin/Expert với badge "Official", nội dung được kiểm duyệt và đảm bảo chất lượng. |
| 3 | Bình luận & Thảo luận | Bình luận, trả lời (replies), và thảo luận trên cả posts và articles. |
| 4 | Tương tác xã hội | Like, bookmark, share bài viết; follow/unfollow người dùng khác. |
| 5 | Báo cáo nội dung | Báo cáo bài viết hoặc bình luận không phù hợp (spam, nội dung xấu, sai lệch) vào reports queue. |
| 6 | Tìm kiếm & Lọc | Tìm bài viết theo từ khóa, tag, loài cây, hoặc bộ lọc (date range, popularity, author). |

[Cần hình: Ảnh minh họa màn hình Cộng đồng / Bảng tin]
[Cần hình: Ảnh minh họa `CreateArticleScreen` từ repo (Giao diện Tạo Bài viết)]
...
| 5 | Quản lý Disease & Treatment Templates | CRUD cho disease templates (symptoms, treatments), treatment templates (steps, materials), plant care templates (watering, fertilizing). |
| 6 | Quản lý Plant Templates | CRUD cho plant species templates (scientific name, care requirements, common diseases). |
| 7 | Thống kê & Dashboard | Dashboard hiển thị: Total users, Active treatments, Pending articles, Reports queue, Engagement metrics, User growth chart. |
| 8 | Audit Log Viewer | Xem lịch sử các hành động admin (approve/reject articles, ban users, delete content) với timestamp và reason. |

[Cần hình: Ảnh minh họa dashboard admin (thống kê)]

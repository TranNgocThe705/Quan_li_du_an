...
export const submitTaskForApproval = asyncHandler(async (req, res) => {
  ...
  // Nếu không có approval request (không có policy), tạo mặc định với Team Lead
  if (!task.approvalRequests || task.approvalRequests.length === 0) {
    console.log('📝 No policy found, creating default approval request with Team Lead');
    
    // Tìm Team Lead của project
    const teamLead = await ProjectMember.findOne({
    ...
    if (teamLead && teamLead.userId) {
      task.approvalRequests = [{
        ...
        approvers: [teamLead.userId._id],
        status: 'PENDING'
      }];
      
      console.log('✅ Created approval request for Team Lead:', teamLead.userId.name);
    } else {
      // Fallback: Nếu không tìm thấy Team Lead, cho phép ai cũng approve
      console.log('⚠️ No Team Lead found, allowing any project member to approve');
      const projectMembers = await ProjectMember.find({
        projectId: task.projectId._id
      }).populate('userId');
      
      const memberIds = projectMembers
        .filter(m => m.userId && m.userId._id.toString() !== task.assigneeId._id.toString())
        .map(m => m.userId._id);
  ...
  if (latestRequest?.approvers) {
    for (const approverId of latestRequest.approvers) {
      await Notification.create({
        userId: approverId,
        fromUserId: task.assigneeId._id,
        type: 'TASK_COMPLETED',

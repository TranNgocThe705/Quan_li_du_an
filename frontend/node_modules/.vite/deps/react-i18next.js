import {
  keysFromSelector
} from "./chunk-JCWPEZ5J.js";
import {
  require_shim
} from "./chunk-UZQDXWZC.js";
import {
  require_react
...
var escapeLiteralLessThan = (str, keepArray = [], knownComponentsMap = {}) => {
  ...
  while (i2 < str.length) {
    if (str[i2] === "<") {
      ...
      const closingMatch = str.slice(i2).match(/^<\/(\d+|[a-zA-Z][a-zA-Z0-9-]*)>/);
      if (closingMatch) {
      ...
      if (!isValidTag) {
        const openingMatch = str.slice(i2).match(/^<(\d+|[a-zA-Z][a-zA-Z0-9-]*)(\s+[\w-]+(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?)*\s*(\/)?>/);
        if (openingMatch) {
...
var useTranslation = (ns, props = {}) => {
  ...
  const namespaces = (0, import_react7.useMemo)(() => {

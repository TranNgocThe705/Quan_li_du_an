        ...
        <div className="mt-6 px-3">
            ...
            <div className="space-y-1 px-3">
                {projects.map((project) => (
                    <div key={project._id}>
                        <button onClick={() => toggleProject(project._id)} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-colors duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-gray-900 dark:hover:text-white" >
                            <ChevronRightIcon className={`size-3 text-gray-500 dark:text-zinc-400 transition-transform duration-200 ${expandedProjects.has(project._id) && 'rotate-90'}`} />
                        ...
                        {expandedProjects.has(project._id) && (
                            <div className="ml-5 mt-1 space-y-1">
                                {getProjectSubItems(project._id).map((subItem) => {

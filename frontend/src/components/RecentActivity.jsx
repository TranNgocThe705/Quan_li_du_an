...
const RecentActivity = () => {
    ...
    // Get latest 10 tasks, sorted by updatedAt
            ...
            <div className="p-0">
                {tasks.length === 0 ? (
                    <div className="p-12 text-center">
                        ...
                        <p className="text-zinc-600 dark:text-zinc-400">Chưa có hoạt động nào</p>
                    </div>
                ) : (
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {tasks.map((task) => {
                            ...
                            return (
                                <div key={task._id} className="p-6 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400">
                                                <span className="capitalize">{task.type?.toLowerCase() || 'task'}</span>
                                                {task.assigneeId && (
                                                    <div className="flex items-center gap-1">
                                                        <div className="w-4 h-4 bg-zinc-300 dark:bg-zinc-700 rounded-full flex items-center justify-center text-[10px] text-zinc-800 dark:text-zinc-200">
                                                            {task.assigneeId.name?.[0]?.toUpperCase() || 'U'}
                                                        </div>
                                                        {task.assigneeId.name}
                                                    </div>
                                                )}
                                                <span>
                                                    {task.updatedAt ? format(new Date(task.updatedAt), "MMM d, h:mm a") : 'Gần đây'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
